...
class StickyDotBezierCurveIndicator(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        mPath.lineTo(
            mNextTransitionalCircle1.centerX,
            mMaxCircleRadius - mNextTransitionalCircle1.radius
        )
        mPath.quadTo(
            mCurTransitionalCircle1.centerX + (mNextTransitionalCircle1.centerX - mCurTransitionalCircle1.centerX) / 2f,
            mMaxCircleRadius,
            mCurTransitionalCircle1.centerX,
            mMaxCircleRadius - mCurTransitionalCircle1.radius
        )
        mPath.lineTo(
            mCurTransitionalCircle1.centerX,
            mMaxCircleRadius + mCurTransitionalCircle1.radius
        ...
        if (mCurTransitionalCircle2.radius > 0f && mNextTransitionalCircle2.radius > 0f) {
            ...
            mPath.lineTo(
                mNextTransitionalCircle2.centerX,
                mMaxCircleRadius - mNextTransitionalCircle2.radius
            )
            mPath.quadTo(
                mCurTransitionalCircle2.centerX + (mNextTransitionalCircle2.centerX - mCurTransitionalCircle2.centerX) / 2f,
                mMaxCircleRadius,
                mCurTransitionalCircle2.centerX,
                mMaxCircleRadius - mCurTransitionalCircle2.radius
            )
            mPath.lineTo(
                mCurTransitionalCircle2.centerX,
                mMaxCircleRadius + mCurTransitionalCircle2.radius
    ...
    override fun onPageScrolled(position: Int, positionOffset: Float, positionOffsetPixels: Int) {
        ...
        mCurTransitionalCircle1.centerX =
            currentCircleX + distance * mStartInterpolator.getInterpolation(positionOffset)
        mCurTransitionalCircle1.radius =
            mMaxCircleRadius + (mMinCircleRadius - mMaxCircleRadius) * mEndInterpolator.getInterpolation(
                positionOffset
            )

        mNextTransitionalCircle1.centerX =
            currentCircleX + distance * mEndInterpolator.getInterpolation(positionOffset)
        mNextTransitionalCircle1.radius =
        ...
        if (position == mDataCount - 1) {
            ...
            mCurTransitionalCircle2.centerX =
                beforeFirstCircleX + distance * mStartInterpolator.getInterpolation(positionOffset)
            mCurTransitionalCircle2.radius =
                mMaxCircleRadius + (mMinCircleRadius - mMaxCircleRadius) * mEndInterpolator.getInterpolation(
                    positionOffset
                )

            mNextTransitionalCircle2.centerX =
                beforeFirstCircleX + distance * mEndInterpolator.getInterpolation(positionOffset)
            mNextTransitionalCircle2.radius =
                mMinCircleRadius + (mMaxCircleRadius - mMinCircleRadius) * mStartInterpolator.getInterpolation(
                    positionOffset
                )
        }
